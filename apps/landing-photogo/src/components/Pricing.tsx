    ...
    <section id="pricing" className="border-t border-ink-900/5 py-20 dark:border-paper-100/5 sm:py-32">
      <div className="container-wide">
        ...
        <div className="mt-16 grid gap-6 lg:grid-cols-3">
          {plans.map((plan, idx) => (
            <FadeInSection key={plan.name} delay={`delay-${(idx + 1) * 100}`}>
              <div
                className={`relative flex h-full flex-col rounded-3xl border p-8 transition hover:-translate-y-1 ${
                  plan.highlighted
                    ? "border-sunset-500 bg-gradient-to-br from-sunset-500/5 to-transparent shadow-xl hover:shadow-2xl"
                    : "border-ink-900/5 bg-paper-50 hover:shadow-lg dark:border-paper-100/5 dark:bg-ink-900"
                }`}
              >
                {plan.highlighted && (
                  <span className="absolute -top-3 left-1/2 -translate-x-1/2 animate-pulse-soft rounded-full bg-sunset-500 px-3 py-1 text-xs font-medium text-white shadow-md">
                    Mais popular
                  </span>
                )}
